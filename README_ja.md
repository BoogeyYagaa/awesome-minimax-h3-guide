...
[用途からレシピを選ぶ](docs/use-case-matrix.md) · [多言語プロンプト例](docs/multilingual-prompting.md) · [API 連携手順](docs/api-workflow.md)

## Flyne AI で MiniMax H3 を使う

| おすすめのブラウザー版 | 無料のオンライン体験 |
|---|---|
| [MiniMax H3](https://flyne.ai/model/minimax-h3/) | [オンラインですぐに試す](https://flyne.ai/free-minimax-h3/) |

ページには登録不要と案内されています。2026-09-21 に確認した無料画面には **5秒・480p** の設定がありました。[5秒の練習](docs/quick-start.md)から始めてください。

## 5秒の練習を試す

FX5 の練習は、作者の動画とは別に作成した学習用プロンプトです。生成結果は未検証です。詳しい解説と練習本文は英語・中国語です。

1. ギャラリーで FX5 の練習を選び、英語または中国語のプロンプトをコピーします。
2. 無料ツールを開き、画像は追加せず、練習に指定された画面比率を選びます。
3. 生成を押し、表示された人間であることの確認を手動で完了します。「Queued」は待機中です。同じ内容を再送せず、ページを開いたまま待ってください。
4. 結果が出たら、形・動き・終わり方を確認します。音声は別途確認してください。

[練習と動画（英語・中国語）](docs/x-community-showcase.md) · [Flyne AI](https://flyne.ai/free-minimax-h3/)

## 動画とプロンプトを見る

X の作例 12 件。動画、原投稿、短い抜粋と解説を掲載。

[練習と動画（英語・中国語）](docs/x-community-showcase.md) · [公式作例](docs/official-h3-examples.md)
...
検索にはレシピの一覧とは別に FX5 の練習も含まれます。`--origin exercise` は練習のみ、`--show-prompt` は英語と中国語の本文を表示します。
...
元の 84 件の著作権表示と MIT ライセンスは Flaq AI に帰属します。軽い言い換えを独自制作とは扱いません。新規案は未検証です。README は 8 言語、詳しいガイドは主に英語で、一部に中国語や多言語の説明があります。プロンプト本文は英語です。Flyne AI の料金・入力モード・利用条件は現在の製品画面で確認してください。
